<template>
  <div class="w-full" >
    <!-- Title of crud -->
    <div class="flex w-full title py-2 px-4 border-b z-50 mt-12 bg-gray-100" >
      <div class="submenu-icon h-6 flex w-1/2">
        ...
        <div class="submenu-icon-title text-xs flex-grow  text-left leading-6 font-moul" v-html="model.title" ></div>
      </div>
      <div class="flex justify-end h-6 w-1/2" >
        ...
        <n-tooltip trigger="hover">
          <template #trigger>
            <svg 
              class="flex-none w-6 h-6 ml-2 text-blue-500 cursor-pointer"
    ...
    <Transition name="fade" >
      <div v-if="Array.isArray( table.records.matched ) && table.records.matched.length > 0 " class="vcb-table-panel flex">
        <div v-for="(folder, index) in table.records.matched" :key='index' class="vcb-table-row bg-white text-left relative mb-8 border w-48 m-1 p-2 rounded cursor-pointer hover:scale-110 transform-gpu duration-300 " style="overflow:hidden; text-wrap: nowrap ; text-overflow: ellipsis;" >
          ...
          <div class="record-actions-panel flex mb-2 mt-8" >
            ...
            <n-tooltip v-if="folder.regulators !== undefined && folder.regulators.length " trigger="hover">
              <template #trigger>
                ...
                @click="$router.push('/folders/'+folder.id+'/regulators')" title="ឯកសារកំពុងបើកជាសាធារណ" 
              ...
              ឯកសារក្នុងថត <span class="text-xs font-bold -mt-1 " >{{ folder.regulators !== undefined && folder.regulators.length > 0 ? getKhmer( folder.regulators.length ) : '' }}</span>
            </n-tooltip> 
          </div>
        </div>
